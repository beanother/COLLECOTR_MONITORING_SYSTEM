...
def main():
    ...
    with tf.name_scope('final_training_ops'):
        ...
        logits = tf.matual(bottleneck_input, weights) + biases
    ...
    with tf.name_scope('evaluation'):
        correct_prediction = tf.equal(tf.argmax(final_tensor, 1), tf.arg_max(ground_truth_input))
    ...
    for i in range(STEPS):
        ...
        test_bottlenecks, test_ground_truth = get_test_bottlenecks(sess, image_lists, n_classes, jpeg_data_tensor,
                                                                   bottleneck_tensor)
        test_accuracy = sess.run(evalution_step,
